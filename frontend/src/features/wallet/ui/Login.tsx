...
import { AlertCircle, ArrowRight, BadgeCheck, Check, Clock3, Copy, Lock, Wallet } from "lucide-react";
...
function ScopedFields(props: ScopedFieldsProps) { const { role } = props; return <><Field label="University"><select value={props.universityCode} onChange={(e) => props.setUniversityCode(e.target.value)} className="input" required disabled={props.loadingUniversities}><option value="">{props.loadingUniversities ? "Loading approved universities…" : "Choose an approved university"}</option>{props.approvedUniversities.map((u) => <option key={u.code} value={u.code}>{u.name} ({u.code})</option>)}</select></Field>{role === "Student" && <><Field label="Department"><input className="input" value={props.department} onChange={(e) => props.setDepartment(e.target.value)} /></Field><Field label="Program"><input className="input" value={props.program} onChange={(e) => props.setProgram(e.target.value)} /></Field><Field label="Graduation year"><input className="input" type="number" value={props.graduationYear} onChange={(e) => props.setGraduationYear(e.target.value)} /></Field><Field label="Student ID"><input className="input" value={props.studentIdentifier} onChange={(e) => props.setStudentIdentifier(e.target.value)} /></Field></>}{role === "Merchant" && <><Field label="Business name"><input className="input" value={props.businessName} onChange={(e) => props.setBusinessName(e.target.value)} /></Field><Field label="Category"><select className="input" value={props.merchantCategory} onChange={(e) => props.setMerchantCategory(e.target.value)}><option value="1">Retail</option><option value="2">Food / Canteen</option><option value="3">Services</option><option value="4">Other</option></select></Field><Field label="Business description"><textarea className="input" value={props.businessDescription} onChange={(e) => props.setBusinessDescription(e.target.value)} /></Field></>}{role === "EventOrganizer" && <><Field label="Club / organization name"><input className="input" value={props.organizationName} onChange={(e) => props.setOrganizationName(e.target.value)} /></Field><Field label="Organization description"><textarea className="input" value={props.organizationDescription} onChange={(e) => props.setOrganizationDescription(e.target.value)} /></Field></>}</>; }
...
function Error({ message }: { message: string }) { return <p className="rounded-lg border border-red-200 bg-red-50 p-3 text-xs text-red-700">{message}</p>; }
