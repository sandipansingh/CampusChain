      ...
      return <button key={listing.id} onClick={() => onSelectItem(String(listing.id))} className="text-left bg-card border border-border rounded-xl hover:border-foreground flex flex-col justify-between overflow-hidden">
        <div className="p-5 flex-1">
          <div className="flex justify-between items-start gap-3 mb-3">
            <h4 className="text-sm font-bold line-clamp-2 leading-snug" title={listing.title}>{listing.title}</h4>
            {listing.escrow_enabled && <span className="px-2 py-0.5 text-[9px] font-bold border border-border rounded-full flex items-center gap-1 shrink-0"><Lock className="h-2.5 w-2.5" />Escrow</span>}
          </div>
          <div className="aspect-[4/3] bg-muted/30 rounded-lg flex items-center justify-center text-muted-foreground border border-border/40 overflow-hidden relative">
            {listing.imageUrl ? (
              <img
            ...
            <div className={`fallback-icon flex items-center justify-center ${listing.imageUrl ? "hidden" : ""}`}>
