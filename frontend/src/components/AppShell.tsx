...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="h-screen bg-[#f4f6f8] text-slate-800 flex p-6 gap-6 font-sans antialiased overflow-hidden">
      ...
      {!address && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/10 backdrop-blur-sm p-6">
          <div className="bg-white rounded-[24px] p-8 max-w-md w-full text-center flex flex-col items-center gap-6 shadow-sm">
            ...
            <Link
              href="/"
              className="text-xs font-semibold text-slate-700 hover:text-accent tracking-tight"
            >
      ...
      {/* Left Sidebar - Desktop - Floating style (borderless) */}
      <aside className="w-64 bg-white rounded-[24px] hidden lg:flex flex-col shrink-0 h-full shadow-sm">
        {/* Logo Brand Header */}
        ...
        <nav className="flex-1 overflow-y-auto px-4 py-6 flex flex-col gap-6">
          {menuItems.map((group) => (
            <div key={group.group} className="flex flex-col gap-2">
              <span className="text-[10px] tracking-wider text-slate-700 font-bold uppercase px-3">
                {group.group}
              </span>
                      ...
                      <div className="flex items-center gap-3">
                        <Icon
                          className={`w-4 h-4 shrink-0 ${
                            isActive ? "text-white" : "text-slate-750 group-hover:text-slate-900"
                          }`}
                        />
                        <span>{item.name}</span>
                      </div>
        ...
        <div className="p-4 bg-slate-50/50 flex flex-col gap-3 rounded-b-[24px]">
          ...
          <button
            ...
            className="flex items-center justify-center gap-2 w-full py-2.5 bg-white text-slate-600 text-xs font-semibold rounded-xl hover:bg-red-50 hover:text-red-600 transition-all duration-200"
          >
      ...
      {/* Right Dashboard Area (Header + Main Content) - Floating Layout (borderless) */}
      <div className="flex-1 flex flex-col gap-6 min-w-0 h-full">
        {/* Top Header - Floating Card */}
        <header className="h-20 bg-white rounded-[24px] flex items-center justify-between px-6 md:px-8 shrink-0 shadow-sm">
          ...
          <div className="hidden md:flex items-center bg-slate-50 rounded-2xl px-4 py-2.5 w-80 focus-within:border-slate-300 transition-all">
            <Search className="w-4 h-4 text-slate-700 mr-2.5 shrink-0" />
            <input
              type="text"
            ...
            />
            <span className="text-[9px] font-semibold text-slate-700 bg-white border border-slate-200 rounded px-1.5 py-0.5 ml-2 shrink-0">
              K ⌘
            </span>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            {address && (
              <div className="hidden md:flex flex-col items-end border-r border-slate-100 pr-4 mr-2">
                <span className="text-[9px] tracking-wider text-slate-700 font-bold uppercase">
                  Balance
                </span>
            ...
            <div className="flex items-center gap-2">
              <Link href="/notifications">
                <button className="relative w-9 h-9 rounded-full bg-white flex items-center justify-center text-slate-500 hover:bg-slate-50 transition-all shadow-sm">
                  <Bell className="w-4 h-4" />
                </button>
              </Link>
              <Link href="/help">
                <button className="w-9 h-9 rounded-full bg-white flex items-center justify-center text-slate-500 hover:bg-slate-50 transition-all shadow-sm">
        ...
        <main className="flex-1 overflow-y-auto pr-1 animate-in fade-in duration-300 flex flex-col justify-between">
          ...
          {/* App Dashboard Footer (borderless) */}
          <footer className="py-6 mt-12 bg-white rounded-t-[24px] px-6 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-sm">
            ...
            <div className="flex gap-6">
              <a
                ...
                className="text-[10px] font-semibold text-slate-700 hover:text-accent uppercase tracking-wider"
              >
                Architecture
              </a>
              <a
